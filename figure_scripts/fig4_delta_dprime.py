...
lax1 = plt.subplot2grid((2, 2), (1, 0))
lax2 = plt.subplot2grid((2, 2), (1, 1))
...
for g in grouped:
    ...
    lax1.plot(cos.bin_edges[1:], cos_val_delt, color='grey', zorder=2)
    lax2.plot(du.bin_edges[1:], du_val_delt, color='grey', zorder=2)
    ...
    lax1.plot(cos.bin_edges[1:], cos_val, color='lightpink', zorder=1)
    lax2.plot(du.bin_edges[1:], du_val, color='lightpink', zorder=1)

du_val_delt = np.nanmean(np.stack(du_delta), axis=0)
cos_val_delt = np.nanmean(np.stack(cos_delta), axis=0)

lax1.plot(cos.bin_edges[1:], cos_val_delt, lw=3, color='k', label=r"$\Delta d'^{2}$", zorder=4)
lax2.plot(du.bin_edges[1:], du_val_delt, lw=3, color='k', label=r"$\Delta d'^{2}$", zorder=4)

du_val = np.nanmean(np.stack(du_dp), axis=0)
cos_val = np.nanmean(np.stack(cos_dp), axis=0)

lax1.plot(cos.bin_edges[1:], cos_val, lw=3, color='magenta', label=r"$d'^{2}$", zorder=3)
lax2.plot(du.bin_edges[1:], du_val, lw=3, color='magenta', label=r"$d'^{2}$", zorder=3)

lax1.set_xlabel(alab.COSTHETA, color=color.COSTHETA)
lax1.spines['bottom'].set_color(color.COSTHETA)
lax1.yaxis.label.set_color(color.COSTHETA)
lax1.tick_params(axis='x', colors=color.COSTHETA)
lax2.set_xlabel(alab.SIGNAL, color=color.SIGNAL)
lax2.spines['bottom'].set_color(color.SIGNAL)
lax2.yaxis.label.set_color(color.SIGNAL)
lax2.tick_params(axis='x', colors=color.SIGNAL)

lax1.legend(frameon=False)
lax1.set_ylabel(r"$\Delta d'^{2}$ / $d'^{2}$"+ "\n (z-score)")
