...
for site in sites:
    ...
    if len(stim) == 0:
        ...
        _df_sim1['state_diff'] = (_df_sim1['bp_dp'] - _df_sim1['sp_dp']) / _df_sim1['dp_opt_test']
        _df_sim1['site'] = site
        df_sim1.append(_df_sim1)

        _df_sim2 = _df_sim2.loc[pd.IndexSlice[stim, 2], :]
        _df_sim2['state_diff'] = (_df_sim2['bp_dp'] - _df_sim2['sp_dp']) / _df_sim2['dp_opt_test']
        _df_sim2['site'] = site
...
df.plot.hexbin(x='dU_mag'+estval, 
                  ...
                  C='sim1', 
                  gridsize=nbins, ax=s1ax, cmap=cmap, vmin=-2, vmax=2) 
...
df.plot.hexbin(x='dU_mag'+estval, 
                  y='cos_dU_evec'+estval, 
                  C='sim2', 
                  gridsize=nbins, ax=s2ax, cmap=cmap, vmin=-2, vmax=2) 
