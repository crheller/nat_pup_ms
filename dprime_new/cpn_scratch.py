...
for batch, site in zip(batches, sites):
    ...
    _df['site'] = site
    _df['sp_noise_mag'] = results.array_results['sp_evals'].loc[pd.IndexSlice[stim, ndim], 'mean'].apply(lambda x: x[0])
    _df['bp_noise_mag'] = results.array_results['bp_evals'].loc[pd.IndexSlice[stim, ndim], 'mean'].apply(lambda x: x[0])
...
# motivated by observation that noise correlations tend to go in opposite direction for many
# pairs of CPN neurons, verify that "diversity" in decoding effects is not due to this
# opposite noise corr. effect.

# only show cases where noise mag gets smaller in big pupil
df = df[df.bp_dp < df.sp_dp]
